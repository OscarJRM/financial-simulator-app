  ...
  export function InvestmentRequestForm({ userId }: InvestmentRequestFormProps) {
    ...
    /**
     * Verificación facial con DeepStack usando selfie del perfil
     */
    const performFaceVerificationWithDeepStack = async (newSelfieFile: File): Promise<{ isMatch: boolean; confidence: number }> => {
      console.log('🎭 INICIANDO VERIFICACIÓN FACIAL CON DEEPSTACK');

      try {
        // 1. Obtener selfie del perfil desde la API
        const profileSelfieUrl = await fetchProfileSelfie();
        console.log('✅ Selfie del perfil obtenida:', profileSelfieUrl.substring(0, 100) + '...');

        // 2. Crear URL temporal para el nuevo archivo
        const newSelfieUrl = URL.createObjectURL(newSelfieFile);
        ...
        URL.revokeObjectURL(newSelfieUrl);

        console.log('🎉 RESULTADO VERIFICACIÓN DEEPSTACK:', {
          coincide: result.isMatch,
        ...
        return {
          ...
          confidence: result.confidence / 100 // Convertir a decimal para consistencia
        };

      } catch (error: any) {
        console.error('❌ ERROR en verificación facial con DeepStack:', error);
        throw error;
      }
    };

    /**
     * Obtener selfie del perfil del usuario
     */

    const fetchProfileSelfie = async (): Promise<string> => {
      console.log('🔍 Buscando selfie del perfil para userId:', userId);

      try {
        const response = await fetch(`/api/users/${userId}/selfie`);

        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.error || 'Error obteniendo selfie del perfil');
        }

        const data = await response.json();

        console.log('📄 Respuesta completa de /api/users/selfie:', data);
        console.log('📸 Selfie URI tipo:', typeof data.selfie_uri);
        console.log('📸 Selfie URI inicio:', data.selfie_uri?.substring(0, 100));

        if (!data.selfie_uri) {
          throw new Error('No se encontró selfie en el perfil del usuario');
        }

        console.log('✅ Selfie del perfil obtenida correctamente');
        return data.selfie_uri;
      } catch (error: any) {
        console.error('💥 Error obteniendo selfie del perfil:', error);
        throw error;
      }
    };

    /**
     * Manejar verificación facial con DeepStack
     */
    const handleFaceVerificationWithDeepStack = async (newSelfieFile: File) => {
      console.log('🎭 Iniciando verificación facial con DeepStack...');
      ...
      try {
        const result = await performFaceVerificationWithDeepStack(newSelfieFile);
    ...
    const handleSelfieUpload = async (file: File) => {
      ...
      try {
        ...
        // Verificar automáticamente usando DeepStack
        await handleFaceVerificationWithDeepStack(file);
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="text-sm space-y-1">
                        ...
                        {simulatorData.monto && formData.ingresos && formData.egresos && (
                          <div className={`font-medium ${(parseFloat(formData.ingresos) - parseFloat(formData.egresos)) >= (parseFloat(simulatorData.monto) * 0.1)
                            ? 'text-green-600'
                            : 'text-red-600'
                            }`}>
